Shader "HTML2VRC/UI Radial Gradient"
{
    Properties
    {
        [PerRendererData] _MainTex ("Sprite Texture", 2D) = "white" {}
        _GradientTex ("Gradient LUT", 2D) = "white" {}
        _GradientCenter ("Gradient Center", Vector) = (0, 0, 0, 0)
        _GradientRadius ("Gradient Radius", Vector) = (50, 50, 0, 0)
        _RectSize ("Rect Size", Vector) = (100, 100, 0, 0)
        _CornerRadii ("Corner Radii", Vector) = (0, 0, 0, 0)
        _Color ("Tint", Color) = (1, 1, 1, 1)
        _StencilComp ("Stencil Comparison", Float) = 8
        _Stencil ("Stencil ID", Float) = 0
        _StencilOp ("Stencil Operation", Float) = 0
        _StencilWriteMask ("Stencil Write Mask", Float) = 255
        _StencilReadMask ("Stencil Read Mask", Float) = 255
        _ColorMask ("Color Mask", Float) = 15
        [Toggle(UNITY_UI_ALPHACLIP)] _UseUIAlphaClip ("Use Alpha Clip", Float) = 0
    }

    SubShader
    {
        Tags
        {
            "Queue" = "Transparent"
            "IgnoreProjector" = "True"
            "RenderType" = "Transparent"
            "PreviewType" = "Plane"
            "CanUseSpriteAtlas" = "True"
        }

        Stencil
        {
            Ref [_Stencil]
            Comp [_StencilComp]
            Pass [_StencilOp]
            ReadMask [_StencilReadMask]
            WriteMask [_StencilWriteMask]
        }

        Cull Off
        Lighting Off
        ZWrite Off
        ZTest [unity_GUIZTestMode]
        Blend SrcAlpha OneMinusSrcAlpha
        ColorMask [_ColorMask]

        Pass
        {
            Name "Default"
            CGPROGRAM
            #pragma vertex vert
            #pragma fragment frag
            #pragma target 2.0
            #pragma multi_compile_local _ UNITY_UI_CLIP_RECT
            #pragma multi_compile_local _ UNITY_UI_ALPHACLIP

            #include "UnityCG.cginc"
            #include "UnityUI.cginc"

            struct appdata_t
            {
                float4 vertex : POSITION;
                float4 color : COLOR;
                float2 texcoord : TEXCOORD0;
                UNITY_VERTEX_INPUT_INSTANCE_ID
            };

            struct v2f
            {
                float4 vertex : SV_POSITION;
                fixed4 color : COLOR;
                float4 worldPosition : TEXCOORD0;
                float2 localPosition : TEXCOORD1;
                UNITY_VERTEX_OUTPUT_STEREO
            };

            sampler2D _GradientTex;
            fixed4 _Color;
            float4 _GradientCenter;
            float4 _GradientRadius;
            float4 _RectSize;
            float4 _CornerRadii;
            float4 _ClipRect;

            float RoundedRectAlpha(float2 localPosition)
            {
                float right = step(0.0, localPosition.x);
                float top = step(0.0, localPosition.y);
                float bottomRadius = lerp(_CornerRadii.w, _CornerRadii.z, right);
                float topRadius = lerp(_CornerRadii.x, _CornerRadii.y, right);
                float radius = lerp(bottomRadius, topRadius, top);
                float2 halfSize = max(_RectSize.xy * 0.5, float2(0.0001, 0.0001));
                float2 distanceToCorner = abs(localPosition) - halfSize + radius;
                float distance = length(max(distanceToCorner, 0.0))
                                 + min(max(distanceToCorner.x, distanceToCorner.y), 0.0)
                                 - radius;
                float antialiasWidth = max(fwidth(distance), 0.0001);
                return saturate(0.5 - distance / antialiasWidth);
            }

            v2f vert(appdata_t input)
            {
                v2f output;
                UNITY_SETUP_INSTANCE_ID(input);
                UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(output);
                output.worldPosition = input.vertex;
                output.localPosition = input.vertex.xy;
                output.vertex = UnityObjectToClipPos(output.worldPosition);
                output.color = input.color * _Color;
                return output;
            }

            fixed4 frag(v2f input) : SV_Target
            {
                float2 normalized = (input.localPosition - _GradientCenter.xy)
                                    / max(_GradientRadius.xy, float2(0.0001, 0.0001));
                float position = saturate(length(normalized));
                fixed4 color = tex2D(_GradientTex, float2(position, 0.5)) * input.color;
                color.a *= RoundedRectAlpha(input.localPosition);

                #ifdef UNITY_UI_CLIP_RECT
                color.a *= UnityGet2DClipping(input.worldPosition.xy, _ClipRect);
                #endif

                #ifdef UNITY_UI_ALPHACLIP
                clip(color.a - 0.001);
                #endif

                return color;
            }
            ENDCG
        }
    }
}
