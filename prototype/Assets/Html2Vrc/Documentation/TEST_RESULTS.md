...
- Unity EditMode `Html2Vrc.Tests.UdomPrototypeTests`: **46/46 통과, 실패 0**
...
HTML2VRC 전용 Unity Test Framework 테스트 46개를 실행한다. 검증 범위는 다음과 같다.
...
43. canonical flex wrap·wrap-reverse의 line 분할, row/column gap, alignContent·alignSelf·stretch, line별 grow/max, reverse 축, top-left Rect bake와 nowrap 전환을 검증
44. canonical font resource의 기존 TMP asset 직접 로드와 TTF/OTF source의 안정 dynamic TMP asset 생성, Text·TextInput 적용, round-trip·재생성·fallback 수명주기 검증
45. canonical zIndex의 flex layout/paint 순서 분리, order·absolute flow·음수/양수/동률·외곽 wrapper 정렬, round-trip·재생성·native Layout Group 복원 검증
46. canonical 양축 overflow hidden의 RectMask2D clipping, 내부 상태 round-trip, 반복 재생성·visible 복원 수명주기와 혼합 축·scroll 명시 오류 검증
