...
제한형 HTML의 `box-shadow`는 쉼표 layer마다 `inset`, 2~4개 px length와 hex·transparent·currentColor를 파싱한다. CSS의 첫 layer가 위에 놓이는 규칙을 canonical의 뒤 layer가 위에 놓이는 규칙으로 바꾸기 위해 배열을 역전하며 offset X/Y, blur, spread와 inset 상태는 그대로 보존한다. `none` 전환 시 원본 노드는 유지하고 shadow·layout wrapper만 제거한다.
