...
`@html2vrc/react`는 같은 canonical UDOM을 결정적인 독립형 HTML로 렌더링합니다. 로컬 preview CLI는 UDOM JSON뿐 아니라 UDOM을 default export하는 JS·TS·JSX·TSX source를 격리된 프로세스에서 실행합니다. source, 같은 폴더의 import dependency 또는 상대 asset이 바뀌면 브라우저를 자동으로 새로고침하고 컴파일 오류에서 다음 저장으로 복구합니다. Binding과 event ID는 미리보기에서 실행하지 않고 진단용 metadata로만 노출합니다. React state를 보존하는 HMR은 다음 단계입니다.
