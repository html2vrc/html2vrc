...
제한형 HTML의 `font-weight`, `font-style`, `line-height`, `letter-spacing`, `white-space`, `text-overflow`와 justify/start/end `text-align`도 같은 TMP text 경로를 사용합니다. Font weight는 `normal`, `bold`, 1~1000 정수를 받고 600 이상을 Bold로 정규화하며 italic과 조합할 수 있습니다. `<strong>/<b>`, `<em>/<i>`, `<small>`은 중첩 가능한 구조화 text run으로 보존하고, TMP 태그처럼 보이는 일반 텍스트는 실행하지 않습니다. Unitless·percentage line-height는 최종 font-size로 계산하고, `pre` 계열 공백과 `<br>`의 원래 순서를 보존합니다. Button과 목록의 파생 라벨에도 동일한 글꼴·run·metric·wrapping 설정이 전달됩니다.
...
제한형 HTML의 solid/none `border` shorthand·edge longhand와 px `border-radius`도 canonical SDF paint 경로를 사용합니다. 비대칭 폭·색, `currentColor`, 모서리별 radius, rounded child clipping과 source-order 덮어쓰기를 유지하며 스타일을 제거하면 같은 원본 GameObject에서 생성 overlay와 mask만 정리합니다.

제한형 HTML의 다중 `box-shadow`는 outer/inset, 음수 offset·spread, blur, hex color·`currentColor`를 radius-aware SDF layer로 변환합니다. CSS와 canonical의 반대인 paint 순서를 보정하며, shadow는 flex 크기와 클릭 영역에 영향을 주지 않고 transform과 함께 이동합니다.
...
제한형 HTML의 단일 `conic-gradient`는 선택적 `from` 각도와 `at` 중심, percentage 또는 angle color stop을 canonical 시계 방향 conic paint로 변환합니다. 기본 위쪽 0°·50% 중심, stop fix-up, rounded mask와 gradient/none/단색 간 안정 재생성을 지원합니다.
